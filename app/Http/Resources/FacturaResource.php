...
class FacturaResource extends JsonResource
{
    ...
    public function toArray($request)
    {
        ...
        return [
            ...
            'subtotal' => (double) number_format($factura->subtotal, 2, ','),
            'valor_iva' => (double) number_format($factura->valor_iva, 2, ','),
            'total' => (double) number_format($factura->total, 2, ','),
            'id_opcion_tipo_entrega' => $tipo_entrega->nombre,
            'id_opcion_tipo_pago' => $tipo_pago->nombre,
            'costo_envio' => (double) number_format($this->costo_envio, 2, ','),
